...
| sourceType     | String   | 是           | 根据sourceType确定源站IP列表或者域名。IP和domain回源必须是json格式。ip回源示例："[{'ip':'1.1.1.1','master':1},{'ip':'1.1.1.2','master':2}]"；ip回源时，1表示主IP，2表示备IP；domain回源示例："[{'domain':'www.a.com','priority':'1'},{'domain':'www.b.com','priority':'2'}]" ，domain回源时，priority代表的是域名优先级，1-5代表优先级从高到低；OSS回源"oss.jcloud.com" |
