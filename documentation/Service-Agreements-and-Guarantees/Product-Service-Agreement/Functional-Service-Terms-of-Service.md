...
**本服务条款是京东云网站所有者（本协议中称为“京东云”网址：[www.jdcloud.com](https://www.jdcloud.com/index)）与您就函数服务的相关事项所订立的有效合约。您通过盖章、网络页面点击确认或以其他方式选择接受本服务条款，包括但不限于未点击确认本服务条款而事实上使用了京东云函数服务，即表示您与京东云已达成协议并同意接受本服务条款的全部约定内容。如若双方盖章文本与网络页面点击确认或以其他方式选择接受之服务条款文本，存有不一致之处，以双方盖章文本为准**。
...
1.1 本条款中“服务”指：京东云向您提供[www.jdcloud.com](https://www.jdcloud.com/index)网站上所展示的函数服务以及相关的技术及网络支持服务。
...
函数服务采用后付费。具体扣费规则请查看[www.jdcloud.com](https://www.jdcloud.com/index)上的页面公告且以页面公布的后付费服务当时有效的计费模式、标准为准。
...
如果您的网站使用函数服务，须保证所开办的全部网站均获得国家有关部门的许可或批准；
...
3.1.10 数据备份系您的义务和责任。虽然京东云的函数服务可能会配置必要的具有日常数据备份功能的工具，但并不意味着数据备份是京东云的义务。京东云不保证完全备份用户数据，亦不对用户数据备份工作或结果承担任何责任。
...
3.2.3 若服务涉及函数服务的，京东云仅负责函数服务以下的底层部分及京东云提供的软件的运营维护，即函数服务的相关技术架构及操作系统等。函数服务之上部分由您自行负责。此外，您自行升级操作系统可能会造成宕机等不良影响，请自行把握风险并谨慎操作。

3.2.4 京东云将消除您非人为操作所出现的故障，但因您原因和/或不可抗力以及非京东云控制范围之内的事项除外。

3.2.5 京东云应严格遵守保密义务。

3.2.6 您理解并认可，京东云在必要时将会对您使用的函数服务进行机房迁移。京东云进行上述操作前将提前7个自然日通知您，由于进行上述操作可能需要修改您相关域名的DNS，因此您需在接到京东云通知后按照京东云要求的时间将DNS修改到京东云指定IP上，否则因此造成网站应用无法访问的，由您自行负责。
...
4.3 除发定及京东云和您另行约定外，自本服务条款期满或因任何原因导致本服务条款提前之中之日起的7个自然日内，京东云应继续存储您的数据，逾期将根据函数服务资源池状态随时可能不再保留您的数据，您需自行承担其数据被销毁后引发的一切后果。
...
7.1 服务期限自您创建任一函数 **服务**（指jdcloud.com上所向您展示并提供的服务）成功之日起计算，具体服务期限将根据您实际使用情况计算，并以您所缴纳的款项数额为依据确认服务期限。函数服务创建成功后，您需进行网站开通前准备的（如办理网站经营许可证/网站备案手续等），准备期间包含在服务期限内。
...
7.2.3.1自首次欠费之时起，您仍可继续使用京东云函数服务24小时；如24小时届满，您仍未成功充值并足以支付所欠服务费用的，京东云将暂停提供服务、冻结您账户下函数服务的操作，但仍存储您的数据。

7.2.3.2自京东云暂停服务之时起满7日后（自服务被暂停之日的暂停开始时刻至第7日相同时刻为期限届满），您仍未成功充值并足以支付所欠服务费用的，则京东云将会终止本服务条款并停止为您继续提供服务；同时，您保存在京东云的全部数据将根据函数服务资源池状态随时可能被删除并清空且永不可恢复。
